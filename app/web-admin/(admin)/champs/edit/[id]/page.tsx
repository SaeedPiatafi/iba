...
const EditChampPage = () => {
  ...
  const [originalChampData, setOriginalChampData] =
    useState<ChampData>(initialChampData);
  ...
  useEffect(() => {
    const fetchChampData = async () => {
      if (!champId) {
        showNotification("Champ ID is missing", "error");
        setIsLoading(false);
        setTimeout(() => router.push("/web-admin/champs"), 2000);
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`/api/admin/champs?id=${champId}`);

        if (!response.ok) {
          throw new Error(
            `Failed to fetch champ: ${response.status} ${response.statusText}`,
          );
        }
        ...
        if (!result.success) {
          throw new Error(result.error || "Failed to load champ");
        }
        ...
        // Transform API data with null safety
        const champ = result.data;
        const transformedChamp: ChampData = {
          id: safeString(champ.id),
          name: safeString(champ.name),
          percentage: Number(champ.percentage) || 0,
          image: safeString(champ.image), // Safe handling of null/undefined
          year: Number(champ.year) || new Date().getFullYear(),
          class: safeString(champ.class) || "9",
        };


        // Set the data
        setChampData(transformedChamp);
        setOriginalChampData(transformedChamp);

        // Set image preview - only if image exists and is not empty
        if (transformedChamp.image && safeTrim(transformedChamp.image) !== "") {
          setImagePreview(transformedChamp.image);
        }

        setIsLoading(false);
      } catch (error) {
        showNotification(
          error instanceof Error ? error.message : "Failed to load champ data",
          "error",
        );
        setIsLoading(false);
        setTimeout(() => router.push("/web-admin/champs"), 2000);
  ...
  const removeSelectedFile = () => {
    ...
    const originalImage = safeString(originalChampData.image);
  ...
  const handleInputChange = (
    ...
    if (name === "image") {
      ...
      setSelectedFile(null);
      setImagePreview(safeValue);
    }

    setChampData((prev) => ({
      ...prev,
      [name]: type === "number" 
        ? (safeValue ? parseFloat(safeValue) : 0) 
        : safeValue,
    }));
  };
  ...
  const hasChanges = (): boolean => {
    const fieldsToCheck: (keyof ChampData)[] = ["name", "percentage", "year", "class", "image"];
    
    // Check basic fields
    const basicFieldsChanged = fieldsToCheck.some((field) => {
      const currentValue = champData[field];
      const originalValue = originalChampData[field];
      
      // Handle string comparison with null safety
      if (typeof currentValue === 'string' && typeof originalValue === 'string') {
        return safeTrim(currentValue) !== safeTrim(originalValue);
      }
      ...
      // For other types or mixed types
      return currentValue !== originalValue;
    });
    ...
    return basicFieldsChanged || imageChanged;
  };

  // Validate form
  const validateForm = (): boolean => {
    if (!safeTrim(champData.name)) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    formData.append("id", champData.id);
    formData.append("name", safeTrim(champData.name));
    ...
    } else if (safeTrim(champData.image)) {
      formData.append("image", safeTrim(champData.image));
    }
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || "Failed to update champ");
      }

      // Update original data after successful submission
      setOriginalChampData(champData);
      // Reset file selection after successful update
      if (selectedFile) {
        setSelectedFile(null);
      }

      showNotification("Champ updated successfully!", "success");
      ...
      showNotification(
        error instanceof Error
          ? error.message
          : "Failed to update champ. Please try again.",
        "error",
      );
    } finally {
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `Are you sure you want to delete ${champData.name}? This action cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      // Call the delete API
      const response = await fetch(`/api/admin/champs?id=${champData.id}`, {
        method: "DELETE",
      });
      ...
      showNotification("Champ deleted successfully! Redirecting...", "success");
      ...
      showNotification(
        error instanceof Error
          ? error.message
          : "Failed to delete champ. Please try again.",
        "error",
      );
      setIsSubmitting(false);
    }
  ...
  const handleResetChanges = () => {
    if (window.confirm("Are you sure you want to reset all changes?")) {
      setChampData({ ...originalChampData });
      setSelectedFile(null);
      const originalImage = safeString(originalChampData.image);
      setImagePreview(originalImage);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Update the details for {champData.name || "Champ"}
            </p>
            {hasChanges() && (
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                {/* Current image preview - only show if we have an image */}
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-600 mb-2">
                    {imagePreview || selectedFile ? "Upload new image or enter URL:" : "Upload image or enter URL:"}
                  </p>
